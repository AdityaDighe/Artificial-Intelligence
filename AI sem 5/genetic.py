...
def crossing(li, n):
    ...
    for i in range(0, n, 2):
        ...
        temp2 = li[j]
        crosspoint = cr(temp1)
        print('The crossover for pair ' + str(i) + ' is ' + str(crosspoint))
...
def mutation(li, n):
    ...
    for i in li:
        j = randint(0, n - 1)
        print('For gene ' + str(i) + ', the bit that will be changed is ' + str(j))
        if i[j] == '1':
            i = i[0:j] + '0' + i[j + 1:]
            mut.append(i)
        elif i[j] == '0':
...
for i in range(m):
    ...
    print('----------------------------------------GENERATION', i, '----------------------------------------')
    print("Initial Population\tX Value\tFitness Value\tPopulation\tExpected Count\tActual Count")

    for j in range(n):
        print(crossed[j], '\t\t', dec[j], '\t\t', fit[j], '\t', prob[j], '\t\t', exe[j], '\t\t', ac[j])

    co = pp(crossed, ac, n)
    print('\nThe selected genes for crossover - \n', co)
